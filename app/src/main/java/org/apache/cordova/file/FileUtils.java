...
public class FileUtils extends CordovaPlugin {
   ...
   /**
    ...
    public void TryWriteFile(Uri rootDir, String fileName, String data) throws Exception{
        String uriString = rootDir.toString() + "/" + fileName;
        String nativeURL = resolveLocalFileSystemURI(rootDir.toString()).getString("nativeURL");
        ...
        if(needPermission(nativeURL, WRITE)) {
            ...
            JSONObject obj = getFile(resolveNativeUri(rootDir).toString(), fileName, new JSONObject().put("create", true), false);
            write(inputURL.toString(), data, 0, false);
        }
    }

    public void TryReadFileByChunk(Uri uri, JSONArray args, CallbackContext callbackContext, Filesystem.ReadFileCallback readFileCallback){
        threadhelper( new FileUtils.FileOp( ){
            public void run(JSONArray args) throws IOException, JSONException {
                boolean isNativeUri = false;
                LocalFilesystemURL inputURL = LocalFilesystemURL.parse(uri);
                if (inputURL == null) {
                ...
                try {
                    ...
                    if (fs.exists(inputURL)) {
                        ...
                        JSONObject file = fs.getFileMetadataForLocalURL(inputURL);

                        fs.readFileAtURL(inputURL, 0, file.getInt("size"), readFileCallback);
                    }else {
                        throw new FileNotFoundException();
                    }
                } catch (IllegalArgumentException e) {
    ...
    public void TryReadFile(Uri uri, JSONArray args, CallbackContext callbackContext, NativeSecp256k1Wrap.ReadFileCallback readFileCallback){
        threadhelper( new FileUtils.FileOp( ){
            public void run(JSONArray args) throws IOException, JSONException {
                boolean isNativeUri = false;
                LocalFilesystemURL inputURL = LocalFilesystemURL.parse(uri);
                ...
                try {
                    ...
                    if (fs.exists(inputURL)) {
                        ...
                        fs.readFileAtURL(inputURL, 0, file.getInt("size"), new Filesystem.ReadFileCallback() {
                            public void handleData(InputStream inputStream, String contentType) {
                                try {
                                    ByteArrayOutputStream os = new ByteArrayOutputStream();
                                    final int BUFFER_SIZE = 8192;
                                    ...
                                    readFileCallback.handleData(os.toByteArray());
                                } catch (IOException e) {
                                    LOG.d(LOG_TAG, e.getLocalizedMessage());
                                    callbackContext.sendPluginResult(new PluginResult(PluginResult.Status.IO_EXCEPTION, NOT_READABLE_ERR));
                                }
